...
import { activitySubtypes, ASValue, isASLink, isASObject } from "./activitystreams/types"
import {Activity, ASObject, Extendable, isActivity, JSONLD, LDValue} from "./types"
import { jsonld } from "./util"
import { debuglog, flatten } from "./util"
import { request } from "./util"
import { rdfaToJsonLd } from "./util"
import { ensureArray, followRedirects, jsonldAppend, makeErrorClass, readableToString, sendRequest } from "./util"

import fetch from "node-fetch";
import { createLogger } from "../src/logger"
const logger = createLogger("activitypub")

export const publicCollectionId = "https://www.w3.org/ns/activitystreams#Public"
...
export const objectTargetsNoRecurse = async (
  ...
  const relatedObjects = (await Promise.all(related.map(async (objOrUrl) => {
    ...
    switch (resContentType) {
      ...
      default:
        logger.warn(`Unexpected contentType=${resContentType} of response when fetching ` +
  ...
  const relatedAudience: ASValue[] = relatedObjectTargetedAudience
    ? flattenAnyArrays(relatedObjects.map((ro) => isASObject(ro) && targetedAudience(ro)))
        .filter(Boolean)
    : []
...
exports.clientHeaders = (headers = {}) => {
  ...
  if (Object.keys(headers).map((h) => h.toLowerCase()).includes("accept")) {
    throw new Error(`ActivityPub Client requests can't include custom Accept header. ` +
...
const deliveryErrors = exports.deliveryErrors = {
  ...
  SomeDeliveriesFailed: makeErrorClass("SomeDeliveriesFailed",
    function(msg: string, failures: Error[], successes: string[]) {
  ...
  switch (targetProfileResponse.statusCode) {
    ...
    default:
      throw new deliveryErrors.TargetRequestFailed(
        `Got unexpected status code ${targetProfileResponse.statusCode} when requesting ${target} to fetchProfile`)
  }
...
const deliverActivity = async (
  activity: Activity, target: string, { deliverToLocalhost }: { deliverToLocalhost: boolean }) => {
  // discover inbox
  logger.debug("req inbox discovery " + target)
  const targetProfileResponse = await (async () => {
    try {
      return await followRedirects(Object.assign(url.parse(target), {
        headers: {
          accept: `${ASJsonLdProfileContentType}, text/html`,
        },
      }))
    } catch (error) {
      logger.error(`Error delivering activity to target=${target}. ` +
        `This is normal if the target doesnt speak great ActivityPub.`, error)
  ...
  switch (targetProfileResponse.statusCode) {
    ...
    default:
      throw new deliveryErrors.TargetRequestFailed(
        `Got unexpected status code ${targetProfileResponse.statusCode} when requesting ` +
        `${target} to determine inbox URL`)
  }
  ...
  if (deliveryResponse.statusCode >= 400 && deliveryResponse.statusCode <= 599) {
    ...
    throw new deliveryErrors.DeliveryErrorResponse(
      `${deliveryResponse.statusCode} response from ${inbox}\nResponse Body:\n${deliveryResponseBody}`)
  }
 ...
 */
async function actorInboxesFromBody(body: string, contentType: string): Promise<string[]> {
  ...
  const actorInboxes = flatten(await Promise.all(actorUrls.map(async (actorUrl) => {
    try {
      ...
      return ensureArray(inbox).map((inboxRelativeUrl) => url.resolve(actorUrl, inboxRelativeUrl))
    } catch (error) {
...
async function directInboxFromBody(body: string, contentType: string) {
  ...
  switch (contentType) {
    ...
    case "application/activity+json":
      ...
      const compactedInbox = (compacted["distbin:inbox"] || [])
        .map((o: {id: string}) => typeof o === "object" ? o.id : o)
