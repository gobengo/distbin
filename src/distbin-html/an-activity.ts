...
export const createHandler = ({
  ...
  return async (req: IncomingMessage, res: ServerResponse) => {
    ...
    const activityRes = await sendRequest(
      createHttpOrHttpsRequest(activityUrl),
    );
    ...
    const activityWithoutDescendants = activityWithUrlsRelativeTo(
      incomingActivity,
      externalUrl,
    );
    const repliesUrls = ensureArray(activityWithoutDescendants.replies).map(
      (repliesUrl: string) => {
        return url.resolve(activityUrl, repliesUrl);
      },
    );
    const descendants = flatten(
      await Promise.all(
        repliesUrls.map(repliesUrl =>
          fetchDescendants(
            repliesUrl,
    ...
    const ancestors = await fetchReplyAncestors(
      externalUrl,
      activity,
      internalUrlRewriter(internalUrl, externalUrl),
    );
    ...
    async function fetchDescendants(
      ...
      const repliesCollectionResponse = await sendRequest(
        createHttpOrHttpsRequest(urlRewriter(repliesUrl)),
      );
      if (repliesCollectionResponse.statusCode !== 200) {
        return {
          name: `Failed to fetch replies at ${repliesUrl} (code ${
            repliesCollectionResponse.statusCode
          })`,
        };
      }
      const repliesCollection = JSON.parse(
        await readableToString(repliesCollectionResponse),
      );

      if (repliesCollection.totalItems <= 0) {
        return repliesCollection;
      }
      repliesCollection.items = await Promise.all(
        repliesCollection.items.map(async (replyActivity: Activity) => {
          // activity with resolved .replies collection
          const withAbsoluteUrls = activityWithUrlsRelativeTo(
            replyActivity,
            repliesUrl,
          );
          const { replies } = withAbsoluteUrls;
          const nextRepliesUrl =
            typeof replies === "string"
              ? replies
              : Array.isArray(replies) && replies.length && replies[0];
          return Object.assign(withAbsoluteUrls, {
            replies:
              typeof nextRepliesUrl === "string"
    ...
    res.end(`
      ...
      ${distbinBodyTemplate({ externalUrl })(`
        ...
        ${renderDescendantsSection(
          ensureArray(activity.replies)[0],
          externalUrl,
        )}
...
<iframe
      ...
      ></iframe>
*/
export const renderActivity = (activity: Activity, externalUrl: string) =>
  renderObject(activity, externalUrl);
...
const href = (linkable: URLString | ASLink | ASObject): string => {
  ...
  if (isASLink(linkable)) {
    return linkable.href;
  }
  if (linkable.url) {
    return href(first(linkable.url));
  }
  return;
};
...
export function renderObject(activity: ASObject, externalUrl: string) {
  const object =
    isActivity(activity) && typeof activity.object === "object"
      ? activity.object
      : activity;
  ...
  const activityObject =
    isActivity(activity) &&
    ensureArray(activity.object).filter(
      (o: ASObject | string) => typeof o === "object",
    )[0];
  const mainHtml = (() => {
    try {
      const maybeMarkdown = activity.content
        ? activity.content
        : activityObject &&
  ...
  return `
        ...
        ${ensureArray(
          ...
          .map((attachment: ASObject & HasLinkPrefetchResult) => {
            ...
            switch (attachment.type) {
              case "Link":
                const prefetch: LinkPrefetchResult =
                  attachment["https://distbin.com/ns/linkPrefetch"];
                ...
                if (
                  !(
                    linkPrefetchSuccess &&
                    linkPrefetchSuccess.supportedMediaTypes
                  )
                ) {
                  return "";
                }
                if (
                  linkPrefetchSuccess.supportedMediaTypes.find((m: string) =>
                    m.startsWith("image/"),
                  )
                ) {
                  return (
                    linkPrefetchSuccess.link &&
                    `
    ...
    published ? formatDate(new Date(Date.parse(published))) : "permalink"
  }</a>
          </span>
          &nbsp;
          <span>
            <a href="${externalUrl}?inReplyTo=${encodeHtmlEntities(
    href(activityUrl),
  )}">reply</a>
          </span>
          &nbsp;
...
function formatTags(o: ASObject) {
  const tags = ensureArray(
    (isActivity(o) && typeof o.object === "object" && o.object.tag) || o.tag,
  ).filter(Boolean);
  return tags
    .map(renderTag)
    .filter(Boolean)
  ...
  function renderTag(tag: ASObject) {
    ...
    const tagUrl =
      tag.url || tag.id || (isProbablyAbsoluteUrl(text) ? text : "");
    let rendered;
    if (tagUrl) {
      rendered = `<a href="${encodeHtmlEntities(
        tagUrl,
      )}" class="activity-tag">${safeText}</a>`;
    } else {
...
function formatAttributedTo(activity: ASObject | Activity) {
  ...
  if (typeof attributedTo === "string") {
    ...
    formatted = encodeHtmlEntities(
      attributedTo.name || first(attributedTo.url),
    );
    authorUrl = attributedTo.url;
  }
  if (authorUrl) {
    formatted = `<a rel="author" href="${encodeHtmlEntities(
      href(first(authorUrl)),
    )}">${formatted}</a>`;
  }
  if (!formatted) {
    return;
  ...
  if (location.latitude && location.longitude) {
    imgUrl = [
      ...
      `center=${location.latitude},${
        location.longitude
      }&zoom=11&size=480x300&sensor=false`,
    ].join("");
    linkTo = `https://www.openstreetmap.org/search?query=${location.latitude},${
      location.longitude
    }`;
    ...
    imgUrl = `https://maps.googleapis.com/maps/api/staticmap?center=${
      location.name
    }&size=480x300&sensor=false`;
    linkTo = `https://www.openstreetmap.org/search?query=${location.name}`;
  }
  const glyph = `
    <a class="glyph" ${
      location.name ? `title="${encodeHtmlEntities(location.name)}"` : ""
    }>
      &#127757;
    </a>
  `;
  ...
  return `
    <details>
      ...
      <ul>
        ...
        ${
          location.altitude
            ? `<li>altitude: ${location.altitude}${location.units || "m"}</li>`
            : ""
        }
        ${
          location.radius
            ? `<li>radius: ${location.radius}${location.units || "m"}</li>`
            : ""
        }
...
function formatGenerator(o: ASObject) {
  const object: ASObject =
    isActivity(o) && typeof o.object === "object" && o.object;
  ...
  if (typeof generator === "object") {
    if (generator.name) {
      generatorText = generator.name;
    } else if (generator.id) {
      generatorText = generator.id;
    }
    let generatorUrl;
    if (generator.url) {
      generatorUrl = generator.url;
    } else if (generator.id) {
      generatorUrl = generator.id;
    }
    if (generatorUrl) {
...
function renderDescendantsSection(
...
function renderAncestor(
  ancestor: Activity | LinkPrefetchFailure,
  externalUrl: string,
): string {
  if (ancestor.type === "LinkPrefetchFailure") {
    ...
    return `
      <article class="activity-item">
        <a href="${linkHref}">${linkHref}</a> (${linkFetchFailure.error ||
...
function renderAncestorsSection(
  ancestors: Array<Activity | LinkPrefetchFailure> = [],
  externalUrl: string,
): string {
  if (!ancestors.length) {
    return "";
  ...
  return `
    <div class="ancestors">
      ${
        olderAncestors.length
          ? renderAncestorsSection(olderAncestors, externalUrl)
          : ""
      }
...
async function fetchReplyAncestors(
  ...
  const inReplyTo = flatten(
    ensureArray(activity.object)
  ...
  let parent: Activity;
  try {
    parent = activityWithUrlsRelativeTo(
      await fetchActivity(urlRewriter(parentUrl)),
      parentUrl,
    );
  } catch (err) {
    switch (err.code) {
  ...
  const ancestorsOfParent = await fetchReplyAncestors(
    baseUrl,
    parent,
    urlRewriter,
  );
...
async function fetchActivity(activityUrl: string) {
  ...
  let activityResponse = await sendRequest(
    createHttpOrHttpsRequest(
      Object.assign(url.parse(activityUrlOrRedirect), {
        headers: {
    ...
    switch (activityResponse.statusCode) {
      ...
      case 302:
        const resolvedUrl = url.resolve(
          activityUrl,
          ensureArray(activityResponse.headers.location)[0],
        );
        activityResponse = await sendRequest(
          createHttpOrHttpsRequest(
            Object.assign(url.parse(resolvedUrl), {
              headers: {
                accept: `${ASJsonLdProfileContentType}, text/html`,
              },
            }),
      ...
      default:
        logger.warn(
          "unexpected fetchActivity statusCode",
  ...
  const resContentType = activityResponse.headers["content-type"]
    ? // strip off params like charset, profile, etc
      ensureArray(activityResponse.headers["content-type"])[0]
        .split(";")[0]
        .toLowerCase()
    : undefined;
  switch (resContentType) {
    ...
    default:
      throw new Error(
        "Unexpected fetched activity content-type: " +
          resContentType +
          " " +
          activityUrl +
          " ",
...
function activityWithUrlsRelativeTo(
  activity: Activity,
  relativeTo: string,
): Activity {
  ...
  if (activity.replies) {
    updates.replies = (replies => {
      if (typeof replies === "string" && isRelativeUrl(replies)) {
        return resolveUrl(relativeTo, replies);
      }
      return replies;
    })(activity.replies);
  }
  if (activity.url) {
    updates.url = ensureArray(activity.url).map(u => {
      if (typeof u === "string" && isRelativeUrl(u)) {
        return resolveUrl(relativeTo, u);
      }
