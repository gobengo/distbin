...
export const first = (itemOrList: any) => {
  if (Array.isArray(itemOrList)) { return itemOrList[0] }
  return itemOrList
}
...
export const readableToString = (readable: NodeJS.ReadableStream): Promise<string> => {
  let body: string = ""
...
export const route = (routes: Map<RoutePattern, RouteResponderFactory>,
  ...
  for (const [routePathname, createHandler] of routes.entries()) {
    if (typeof routePathname === "string") {
      // exact match
      if (pathname !== routePathname) { continue }
...
export const sendRequest = (r: http.ClientRequest): Promise<http.IncomingMessage> => {
  return new Promise((resolve, reject) => {
...
export async function followRedirects(requestOpts: ClientRequestArgs, maxRedirects= 5) {
  ...
  followRedirects: while (redirectsLeft > 0) {
    ...
    switch (latestResponse.statusCode) {
      case 301:
      case 302:
        const nextUrl = url.resolve(latestUrl, ensureArray(latestResponse.headers.location)[0])
...
export const createHttpOrHttpsRequest = (urlOrObj: string|UrlObject) => {
  const parsedUrl: UrlObject = (typeof urlOrObj === "string") ? url.parse(urlOrObj) : urlOrObj
  let createRequest
  switch (parsedUrl.protocol) {
    ...
    default:
      ...
      throw new Error(
        "Can't fetch activity with unsupported protocol in URL (only http, https supported): " + activityUrl)
  }
...
export const makeErrorClass = (name: string, setUp?: ((...args: any[]) => void)) => class extends Error {
